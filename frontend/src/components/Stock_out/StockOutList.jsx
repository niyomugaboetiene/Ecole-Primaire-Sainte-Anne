            ...
            <div className="">
                <div className="max-w-7xl mx-auto w-full">
                   <div className="flex justify-between">
                        <h1 className="text-xl font-bold text-center text-sky-500 mb-2">Stock Out List</h1>
                        <button className="bg-sky-500 flex justify-center items-center py-3 px-6 rounded-lg text-white mb-3 hover:bg-sky-600 transition-colors" onClick={() => navigate('/stockOut/AddNew')}><FaPlus /> Add New</button>
                    </div>
                        ...
                        {stockOut?.map((prod, index) => (
                            <tr key={index} className={index % 2 === 0 ? 'bg-sky-200 hover:bg-sky-300 transition-colors' : 'bg-gray-200 hover:bg-gray-300 transition-colors text-gray-800 font-bold'}>
                                ...
                                <td className="flex justify-between">
                                    <Link to={`/stockOut/update/${prod._id}`} className="inline-flex bg-green-300 rounded-lg hover:bg-green-400  transition-colors text-white py-2 px-4 mt-2"><FaEdit /> Update</Link>
