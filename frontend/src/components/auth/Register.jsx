...
const Register = () => {
    ...
    const handleRegister = async () => {
        try {
            ...
            setIsLoading(true);
            const res = await axios.post('http://localhost:5000/auth/sign-in', { UserName, Password }, { withCredentials: true });
        ...
        <div className="bg-sky-200 min-h-screen flex justify-center items-center">
            <div className="bg-white p-2 h-fit w-90 rounded-lg shadow-lg">
                <div>
                    ...
                    <h1 className="text-center text-gray-700 font-bold text-lg">Register</h1>
                <div className="mt-2">
                ...
                <button onClick={handleRegister} className="w-full flex justify-center items-center  mt-6 bg-sky-300 py-2 gap-3 text-white font-bold rounded-full hover:bg-sky-400 transition-colors"><FaUserPlus />Register</button>
            </div>
        </div>
    )
}
